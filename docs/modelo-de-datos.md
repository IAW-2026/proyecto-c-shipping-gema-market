...
| created_at | timestamp | |
| pickup_lat | float | nullable |
| pickup_lng | float | nullable |
| delivery_lat | float | nullable |
| delivery_lng | float | nullable |
| route_distance | float | nullable |
| route_duration | float | nullable |
...
| order_id | string | FK lógica → Buyer App (único) |
| quote_id | string | FK lógica → Shipping App (cotización origen) |
| buyer_id | string | |
...
| weight | decimal | Peso del paquete (kg) |
| dimensions | JSON | `{ "width": 20, "height": 10, "depth": 30 }` |
| pickup_address | JSON | |
| delivery_address | JSON | |
| tracking_code | string | Único |
| status | enum | `waiting_for_courier`, `pending_pickup`, `picked_up`, `in_transit`, `delivered` |
| price | decimal | |
...
| price_per_km | decimal | USD por km |
