/** Cliente HTTP para la API de OpenRouteService: geocoding y cálculo de rutas. */
export type ORSCoordinate = [lng: number, lat: number];

export interface ORSGeocodeFeature {
    geometry: { coordinates: ORSCoordinate; type: string };
    properties: { label: string; confidence: number; [key: string]: unknown };
}
...
export async function getCoordinatesFromAddress(
    ...
    const params = new URLSearchParams({
        api_key: apiKey,
        address: addr,
        locality: address.city ?? "Bahía Blanca",
        region: "Buenos Aires",
        country: "Argentina",
        size: "1",
    });
